...
namespace mana
{
	...
	bool SemanticAnalyzer::SearchSymbolFromName(const std::shared_ptr<SyntaxNode>& node)
	{
		...
		if (node->GetSymbol() == nullptr)
		{
			...
			{
				CompileError("incomplete type name '%s'", node->GetString());
	...
	std::shared_ptr<TypeDescriptor> SemanticAnalyzer::ResolveVariableSize(const std::shared_ptr<SyntaxNode>& node)
	{
		...
		if (node->GetString())
		{
			...
			if (symbol)
			{
				if (symbol->GetClassTypeId() == SymbolEntry::ClassTypeId::CONSTANT_INT)
				{
	...
	void SemanticAnalyzer::ResolveVariableDescription(const std::shared_ptr<SyntaxNode>& node, const SymbolEntry::MemoryTypeId memoryTypeId, const bool isStaticVariable)
	{
		MANA_ASSERT(node);
