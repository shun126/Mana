...
namespace mana
{
	...
	int32_t LocalAddressResolver::Break(int32_t new_pc)
	{
		int32_t old_pc = -1;

		if (mJumpChainTablePointer > 0)
		{
			old_pc = mJumpChainTable[mJumpChainTablePointer].break_chain;
			mJumpChainTable[mJumpChainTablePointer].break_chain = new_pc;
		}
		else
		{
	...
	int32_t LocalAddressResolver::Continue(int32_t new_pc)
	{
		int32_t i;
		int32_t old_pc = -1;
		...
		if (i > 0)
		{
			old_pc = mJumpChainTable[i].continue_chain;
			mJumpChainTable[i].continue_chain = new_pc;
		}
		else
		{
