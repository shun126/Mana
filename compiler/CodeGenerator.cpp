...
namespace mana
{
	...
	void CodeGenerator::generator_add_event(const char* name, generator_event_funtion_type function)
	{
	...
	*/
	...
	/*!
	returnの処理
	@param	func	returnが所属する関数のSymbolEntry
	@param	tree	return文のSyntaxNode
	*/
	void CodeGenerator::generator_return(std::shared_ptr<SymbolEntry> func, std::shared_ptr<SyntaxNode> tree)
	{
		// Error check
		...
		{
			// TODO: Auto type cast
			// tree->SetLeftNode(node_cast(GetTypeDescriptor(), tree->GetLeftNode()));
		...
		// 関数の最後にジャンプ
		symbol_return_address_list = mCodeBuffer->AddOpecodeAndOperand(MANA_IL_BRA, symbol_return_address_list);
	...
	*/
	...
	@param	count	引数の番号
	@param	param	引数のSymbolEntry
	@param	arg		引数のSyntaxNode
	@return	引数の数
	*/
	int32_t CodeGenerator::generate_argument(int32_t count, std::shared_ptr<SymbolEntry> param, std::shared_ptr<SyntaxNode> arg)
	{
		if (param && arg)
		{
			...
			// TODO:cast
			//arg = node_cast(param->GetTypeDescriptor(), arg);
			//type_compatible(param->GetTypeDescriptor(), arg->GetTypeDescriptor());
	...
	* @return	引数のサイズ
	*/
	int32_t CodeGenerator::generator_call_argument_size(int32_t address, std::shared_ptr<SymbolEntry> param, std::shared_ptr<SyntaxNode> arg)
	{
		if (param && arg)
		{
			...
			// TODO::Alignment
			address += (arg->GetMemorySize() + (sizeof(int32_t) - 1)) / sizeof(int32_t);
		}
		return address;
	}
	...
	* @return	引数のアドレス
	*/
	int32_t CodeGenerator::generator_call_argument(int32_t address, std::shared_ptr<SymbolEntry> param, std::shared_ptr<SyntaxNode> arg)
	{
		if (param && arg)
		{
			address = generator_call_argument(address, param->GetNext(), arg->GetRightNode());

			mCodeBuffer->Add<int16_t>(address);
	...
	*/
	void CodeGenerator::generator_call(std::shared_ptr<SyntaxNode> node)
	{
		...
		else if ((node->GetSymbol())->GetClassTypeId() == SymbolEntry::ClassTypeId::NATIVE_FUNCTION)
		{
	...
	* @return	現在のプログラムアドレス
	*/
	int32_t CodeGenerator::generator_condition_core(std::shared_ptr<SyntaxNode> tree)
	{
		// 判別式内に代入式があるか調べます
	...
	@return	現在のプログラムアドレス
	*/
	int32_t CodeGenerator::generator_condition(std::shared_ptr<SyntaxNode> tree, int32_t match)
	{
		...
		return mCodeBuffer->AddOpecodeAndOperand(match ? MANA_IL_BEQ : MANA_IL_BNE, -1);
	}
	...
	DO_RECURSIVE:
		...
		case SyntaxNode::Id::DeclareFunction:
		{
			...
			mSymbolTable->OpenBlock(false);
			{
				...
				//TODO
				//mGlobalSemanticAnalyzer->pre_resolver_resolve(node->GetRightNode());
		...
		case SyntaxNode::Id::Goto:
			...
			MANA_ASSERT(node->GetBodyNode() == nullptr);
			{
				std::shared_ptr<SymbolEntry> symbol = mSymbolFactory->CreateLabel(node->GetString());
		...
		case SyntaxNode::Id::If:
		{
			...
			{
				const int32_t else_begin_address = mCodeBuffer->AddOpecodeAndOperand(MANA_IL_BRA, -1);
		...
		case SyntaxNode::Id::Switch:
		{
			...
			mCodeBuffer->AddOpecodeAndOperand(MANA_IL_BRA, -1);
		...
		case SyntaxNode::Id::Join:
			...
			mSymbolTable->AddJoin(node->GetLeftNode(), node->GetRightNode());
			MANA_ASSERT(node->GetBodyNode() == nullptr);
			break;
		...
		case SyntaxNode::Id::Request:
			mSymbolTable->AddRequest(MANA_IL_REQ, node->GetLeftNode(), node->GetRightNode(), node->GetString());
			MANA_ASSERT(node->GetBodyNode() == nullptr);
			break;
		...
		case SyntaxNode::Id::Array:
			...
			mCodeBuffer->AddOpecodeAndOperand(MANA_IL_PUSH_INTEGER, (node->GetTypeDescriptor())->GetMemorySize());
			...
			// 三項演算子
		{
			...
			pc1 = mCodeBuffer->AddOpecodeAndOperand(MANA_IL_BEQ, -1);
			generator_genearte_code(node->GetLeftNode(), enable_load);
			pc2 = mCodeBuffer->AddOpecodeAndOperand(MANA_IL_BRA, -1);
		...
		case SyntaxNode::Id::Identifier:
			...
			if (node->GetSymbol())
			{
				...
				case SymbolEntry::ClassTypeId::NEW_SYMBOL:
			...
			MANA_ASSERT(node->GetRightNode() == nullptr);
			{
				...
				if (type)
				{
					if (type->GetId() == TypeDescriptor::Id::Struct)
					{
						...
						//while (type)
						{
							for (std::shared_ptr<SymbolEntry> symbol = type->GetSymbolEntry(); symbol; symbol = symbol->GetNext())
							{
								if (strcmp(symbol->GetName(), node->GetString()) == 0 && symbol->GetClassTypeId() == SymbolEntry::ClassTypeId::VARIABLE_ACTOR)
								{
									// variable.member
									mCodeBuffer->AddOpecodeAndOperand(MANA_IL_PUSH_INTEGER, symbol->GetAddress());
									generator_genearte_code(node->GetLeftNode(), false);
									mCodeBuffer->AddOpecode(MANA_IL_ADD_INTEGER);
									if (enable_load)
										generator_resolve_load(node);
					...
					{
						CompileError("reference to non-struct GetTypeDescriptor()");
					}
				}
			}
		ESCAPE:
		...
		case SyntaxNode::Id::NODE_MEMOP:
			/* variable.member */
			mCodeBuffer->AddOpecodeAndOperand(MANA_IL_PUSH_INTEGER, node->etc);
			generator_genearte_code(node->GetLeftNode(), false);
			mCodeBuffer->AddOpecode(MANA_IL_ADD_INTEGER);
			if (enable_load)
			{
