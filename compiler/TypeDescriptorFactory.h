...
namespace mana
{
	class TypeDescriptorFactory : private Noncopyable
	{
	public:
		TypeDescriptorFactory();
		virtual ~TypeDescriptorFactory();

		std::shared_ptr<TypeDescriptor> Get(const TypeDescriptor::Id id) const;
		std::shared_ptr<TypeDescriptor> GetString() const { return mStringTypeDescriptor; }
