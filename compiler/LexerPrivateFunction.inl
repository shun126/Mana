...
bool Lexer::Open(const char* filename, const bool check)
{
	char path[_MAX_PATH];
	...
	if(mana::fullpath(path, filename, _MAX_PATH) == NULL)
	{
		if(mCurrentPath && mCurrentPath[0] != '\0')
	...
	const char* pathPtr = mana::GetSystemHolder().GetStringPool()->Set(path);
...
*/
	...
	{
		if(mCurrentPath && mCurrentPath[0] != '\0')
		{ mana::CompileError("unable to open \"%s\"", pathPtr); }
		else
		{ MANA_PRINT("unable to open \"%s\"\n", pathPtr); }
		return false;
	}
...
bool Lexer::IsOpened(const char* path)
{
...
bool Lexer::Close()
{
	...
	else
	{
		...
		// set currect directory
		mana::splitpath(mCurrentPath, drive, sizeof(drive), dir, sizeof(dir), fname, sizeof(fname), ext, sizeof(ext));
		mana::makepath(path, sizeof(path), drive, dir, "", "");
		mana::chdir(path);
...
const char* Lexer::GetCurrentFilename(void)
{
	return mCurrentPath;
}

void Lexer::SetCurrentFilename(const char* filename)
{
