	...
	void LocalSemanticAnalyzer::AutoCast(const std::shared_ptr<SyntaxNode>& node)
	{
		...
		if (node->GetLeftNode())
		{
			if (node->GetTypeDescriptor()->GetId() != node->GetLeftNode()->GetTypeDescriptor()->GetId())
				node->SetLeftNode(node_cast(node->GetTypeDescriptor(), node->GetLeftNode()));
		}
		if (node->GetRightNode())
		{
			if (node->GetTypeDescriptor()->GetId() != node->GetRightNode()->GetTypeDescriptor()->GetId())
				node->SetRightNode(node_cast(node->GetTypeDescriptor(), node->GetRightNode()));
		}
	#endif
	}
...
DO_RECURSIVE:
		...
		case SyntaxNode::Id::DeclareVariable:
			ResolveVariableDescription(node, SymbolEntry::MemoryTypeId::NORMAL, false);
			// node->GetLeftNode()
			// node->GetRightNode()
		...
		case SyntaxNode::Id::MemberVariable:
		{
			...
			if (type)
			{
				while (type->GetId() == TypeDescriptor::Id::Array)
				{
