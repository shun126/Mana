"""Tests for the Mana documentation tools.
    ...
    python -m unittest discover -s documents/tools
"""
...
class DocumentTree:
    ...
    def __init__(self, directory: Path):
        self.root = directory
        self.document = directory / "documents"
        ...
        self.write("documents/wiki/wiki.yml", WIKI_YML)
        self.write("documents/assets/common/logo_small.png", "png")
        self.write("documents/assets/ja/diagrams/request-flow.svg", "<svg/>")
...
class WikiGenerationTest(unittest.TestCase):
    def setUp(self):
        ...
        for relative, text in JAPANESE_PAGES.items():
            self.tree.write("documents/" + relative, text)
    ...
    def test_unlisted_manuscript_link_is_reported(self):
        self.tree.write(
            "documents/wiki/ja/tutorial/tutorial-request.md",
            "# Action を Request する\n\n[未登録](./tutorial-loops.md)\n",
        )
        self.tree.write("documents/wiki/ja/tutorial/tutorial-loops.md", "# Loops\n")
        self.generate()
...
class SidebarAndTranslationTest(unittest.TestCase):
    ...
    def add_english(self):
        for relative, text in JAPANESE_PAGES.items():
            self.tree.write("documents/" + relative.replace("/ja/", "/en/"), text)
...
class PartialTranslationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for relative in ("wiki/ja/Home.md", "wiki/ja/reference/reference-request.md"):
            self.tree.write(
                "documents/" + relative.replace("/ja/", "/en/"), JAPANESE_PAGES[relative]
            )
        self.tree.apply(self)
        self.config = manadoc.load_wiki_config(manadoc.WIKI_CONFIG)
...
class SiteRootTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(directory.cleanup)
        self.tree = DocumentTree(Path(directory.name))
        self.tree.write("documents/pages/theme/base.html", "<html>{{ content }}</html>")
        self.tree.apply(self)
        self.site = {
            "title": "Mana",
            "default_language": "ja",
...
class AssetTest(unittest.TestCase):
    ...
    def test_assets_of_other_languages_are_not_published(self):
        self.tree.write("documents/assets/en/diagrams/request-flow.svg", "<svg/>")
