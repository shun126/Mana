...
`document/wiki/<language>/` and `document/pages/<language>/` hold the
...
# manuscript reference to `document/assets/ja/diagrams/x.svg` becomes
...
def load_wiki_config(path: Path = WIKI_CONFIG) -> WikiConfig:
    ...
    config.document_root = repository.get("document_root", "document")
    return config
