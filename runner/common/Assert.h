...
#define MANA_BUG(...)		(void)(mana::Trace(mana::TraceLevel::Error, { __FILE__, "(", std::to_string(__LINE__), "): BUG!: ", __VA_ARGS__, "\n" }), std::terminate())
...
#define MANA_ASSERT_MESSAGE(expression, ...) (void)( \
	(!!(expression)) || \
	(mana::Trace(mana::TraceLevel::Error, { #expression, ": ", __FILE__, "(", std::to_string(__LINE__), "): ", __VA_ARGS__, "\n" }), std::terminate(), 0) \
	)
#else
...
#define MANA_VERIFY(expression)	(void)( \
	(!!(expression)) || \
	(mana::Trace(mana::TraceLevel::Error, { __FILE__, "(", std::to_string(__LINE__), "): (", #expression, ")\n" }), std::terminate(), 0) \
	)
#define MANA_VERIFY_MESSAGE(expression, ...) (void)( \
	(!!(expression)) || \
	(mana::Trace(mana::TraceLevel::Error, { __FILE__, "(", std::to_string(__LINE__), "): ", __VA_ARGS__, " (", #expression, ")\n" }), std::terminate(), 0) \
	)
#define MANA_NOT_IMPLEMENTED() (void)( \
	(mana::Trace(mana::TraceLevel::Error, { __FILE__, "(", std::to_string(__LINE__), "): Not implemented!\n" }), std::terminate()) \
	)
