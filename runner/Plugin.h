...
namespace mana
{
    ...
    public:
        ...
        void Regist(const char* directoryName);
	...
	inline void Plugin::Load(const char* fileName)
	{
		auto vm = mVM.lock();
		if (vm == nullptr)
		{
		...
		MODULE module = LOAD_LIBRARY(fileName);
		if (module == nullptr)
		{
			return;
		}
	...
	inline void Plugin::Regist(const char* directoryName)
	{
#if defined(MANA_TARGET_WINDOWS)
		{
			HANDLE handle;
			...
			strcpy_s(entry, _MAX_PATH, directoryName);
